  ...
  describe '.pay' do
    ...
    [
      ...
      it "raises PaymentServiceError when Braintree encounter a #{braintree_error.name}" do
        ...
        amount = 2
        expect(Braintree::Transaction).to receive(:sale).with(payment_method_nonce: nonce, amount: amount).and_raise(braintree_error)
    ...
    it "raises PaymentServiceError when Braintree encounter a Braintree::ConfigurationError" do
        nonce = 'fake-valid-nonce'
        amount = 2
        expect(Braintree::Transaction).to receive(:sale).with(payment_method_nonce: nonce, amount: amount).and_raise(Braintree::ConfigurationError.new('setting', 'message'))
        expect{ Payment.new(nonce, amount).pay }.to raise_error(Payment::PaymentServiceError)
    end
